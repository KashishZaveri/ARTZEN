...
export const getSingleArt = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Server Error: Cannot fetch artwork" });
  }
};

//Fetch all artworks for the logged-in user
export const getMyArts = async (req, res) => {
  try {
    const myArts = await Product.find({ user: req.user.id }).sort({
      createdAt: -1,
    });
...
// Delete an artwork owned by the user
export const deleteMyArt = async (req, res) => {
  try {
    ...
    if (art.user.toString() !== req.user.id.toString()) {
      return res
        .status(401)
        .json({ success: false, message: "Unauthorized: Not your artwork" });
    }
    ...
    res
      ...
      .json({
        success: false,
        message: "Server Error. Could not delete artwork.",
      });
  }
};
...
export const updateMyArt = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "Server Error. Could not update artwork.",
      });
  }
};